...
*Generated by **`bin/wiki-work scan`**. ****Do not hand-edit**** — it is rebuilt from the repository on every scan, so an edit to it is erased. Every row is a live condition, not a checkbox: an item leaves this list when the thing it points at changes, and never before.*

**This file is a required session step, not a suggestion.** Per CLAUDE.md: read `LLM_HANDOFF.md` and `operator-log.md`, run `bin/wiki-work`, do what the operator actually asked for, then come back and drain the obligations from the top. Anything still outstanding when the session ends goes into `LLM_HANDOFF.md` with a reason — never silently.

Nothing here blocks a commit. The gates print what is outstanding and pass anyway; a question asked on Tuesday must not stand in the way of Thursday's typo fix.

## Obligations (141)

Somebody or something is waiting on each of these. Drain from the top.

Priority: **0** failing gate · **1** parked question · **2** staged answer · **3** stale premise · **4** unnormalised portal edit.
...
The campaign. Worked top-down by choice rather than drained, and each line below is a pointer — the file named is where the work itself is written, and the only place to strike an entry.
