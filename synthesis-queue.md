...
Generated by `bin/wiki-climb candidates`. Each entry is a group of pages that are densely interlinked, span two or more domains, and have no page above them. **A cluster is a question, not an instruction:** per SYNTHESIS_SPEC.md step 3, if you cannot write one sentence that is true of every member, not true of the corpus generally, and falsifiable — reject it here with a line of reasoning and move on. A considered non-synthesis is also knowledge.
