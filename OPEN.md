...
Everything the wiki currently admits it does not know, collected from the pages themselves. A gap struck through or marked CLOSED/RESOLVED/SETTLED is filtered out, so this list shrinks only when something is actually settled.
...
Gaps the operator has already answered in a `## Operator answers — pending ingest` block, which nothing has acted on yet. The evidence is in the repo; what is missing is the pass that corrects the page and every page that inherited the gap, then runs `bin/wiki-gaps clear <page>`. Cheapest work here — no new source required, and the answer is already written down.

*Nothing staged.*

## Live contradictions

Pages making incompatible claims, held rather than resolved. These are the highest-value items here: each one is a place the corpus knows it is wrong somewhere and has not yet found out where.
...
Falsifiable claims awaiting a verdict. Scoring one of these is the cheapest new knowledge available, because no new source is required.
